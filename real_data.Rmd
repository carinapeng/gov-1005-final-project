...
respondents <- read_sheet(ss = respondents_link) %>% 
  ...
  mutate(group_size = as.numeric(as.factor(sapply(what_is_your_blocking_group_size_including_yourself, toString))))%>%
  select(-what_is_your_blocking_group_size_including_yourself)
...
new<-respondents %>%
  ...
  filter(!is.na(name))%>%
  select(group_name, group_leader,linking_group,house,is_your_house_the_best_house,group_size, member,name)

part_respondents <- respondents1 %>%
  select(name,c(freshman_dorm:suitemate)) 
...
full_combined <- sheet %>%
  select(name, what_house_were_you_placed_in) %>%
