...
suitemates <- function(community) {
  suite_yes <- which(grepl("Yes", community$suitemates))
  suite_no <- which(grepl("No", community$suitemates))
...
# group size proportions table
  ...
  prop_group_size_tibble
}
```

```{r appr, echo=F}

approval <- function(community) {
  app_yes <- which(grepl(1, community$approval))
  app_no <- which(grepl(0, community$approval))
...
overview <- function(assigned, community){
  ...
  tibble(
    ...
    prop_ethnicity = as.tibble(ethnicity(community)) %>% nest(),
    prop_religion = as.tibble(religion(community)) %>% nest(),
    prop_sexual_orientation = as.tibble(sexual_orientation(community)) %>% nest(),
    prop_group_size = as.tibble(group_size(community)) %>% nest(),
    prop_suitemates = as.tibble(suitemates(community)) %>% nest(),
    prop_suitemates_reasons = as.tibble(suitemates_reasons(community)) %>% nest(),
    prop_approval = as.tibble(approval(community)) %>% nest()
  )
  # prop_ethnicity
  # prop_religion
...
# 2) take each tibble of "assigned" freshmen and generate community summaries neighborhoods
