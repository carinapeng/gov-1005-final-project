...
confidence_interval <- function(community, lower_percentile, upper_percentile){
  percentiles <- tibble(
    percentile = c(lower_percentile, upper_percentile),
    prop_sex = quantile(community %>% ungroup() %>% pull(prop_sex), c(lower_percentile, upper_percentile)),
    prop_international = quantile(community %>% ungroup() %>% pull(prop_international), c(lower_percentile, upper_percentile)),
    prop_varsity = quantile(community %>% ungroup() %>% pull(prop_varsity), c(lower_percentile, upper_percentile)),
    prop_legacy = quantile(community %>% ungroup() %>% pull(prop_legacy), c(lower_percentile, upper_percentile)),
    prop_financial_aid = quantile(community %>% ungroup() %>% pull(prop_financial_aid), c(lower_percentile, upper_percentile)),
    prop_group_size = quantile(community %>% ungroup() %>% pull(prop_group_size), c(lower_percentile, upper_percentile))   
  )
  percentiles
}
```
...
ci_river <- confidence_interval(river, 0.025, 0.975)
ci_river
