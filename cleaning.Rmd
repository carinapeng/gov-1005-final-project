...
# First we begin by listing the number of students in each house

total_uppseclassmen <- 3000
Adams <- 263 / total_uppseclassmen * respondents
Cabot <- 357 / total_uppseclassmen * respondents
...
for (row in 1:nrow(grouped)) {
    if((count + grouped[row,"group_size"] )< 10) {
        grouped[row,"random_house"]<- "Adams"
        count <- count+ grouped[row,"group_size"]
    }
      
    else if(10 <= (count + grouped[row,"group_size"] ) & (count + grouped[row,"group_size"] ) < 20) {
        grouped[row,"random_house"]<- "Cabot"
        count <- count+ grouped[row,"group_size"]
    }
    
    else if (20 <= (count + grouped[row,"group_size"] ) & (count + grouped[row,"group_size"] ) < 30){ grouped[row,"random_house"]<- "Currier"
         count <- count+ grouped[row,"group_size"]}
   
    else grouped[row,"random_house"]<- "Dunster"
}
