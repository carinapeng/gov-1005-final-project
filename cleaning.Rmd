...
# create a function that assign houses
fun_assign <- function(){
...
overview <- function(community = assigned_all){
   ...
   test <- tibble(
    ...
    prop_white = community %>% 
      ...
      pull()/nrow(community))
   test
  convertRowsToList(test, name.list = TRUE, name.vector = FALSE,
  factors.as.char = FALSE, as.vector = FALSE)

  # this will convert the data in overview to a list. One idea is to put the big
  # chunk of assigning function here in overview as well, and has it output a list
  # based on a new sample each time. If we put all these list in a column, then we can 
  # take a look at the nth element in the list in each row and calculate percentile?
  # however, doing it this way we'll have to repeat the process for different communities,
...
# idea, for each community, convert it into a list and add to a table that 
# contains these list
...
table <- tibble(1:1000, overview1 = map(c(square, river_east)), ~overview(.))


```

```{r}
# Need to create a function that assigns 
# blocking groups to houses. That's what needs
# to be redone multiple times. What about defining the
# house communities? Does that need to be done?
# multiple_houses <- tibble(
#   id = 1:3,
#   students = (assigned_all %>% nest()),
#   
# )
# 
# multiple_houses
